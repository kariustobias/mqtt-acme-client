...
func main() {
	...
	if token := c.Subscribe("/acme/server", 0, nil); token.Wait() && token.Error() != nil {
		fmt.Println(token.Error())
		os.Exit(1)
	}
	...
	for {
		...
		if strings.Compare(message, "\n") > 0 {
			...
			token := c.Publish("/acme/server", 0, false, message)
	...
	if token := c.Unsubscribe("/acme/server"); token.Wait() && token.Error() != nil {
		fmt.Println(token.Error())
		os.Exit(1)
	}
